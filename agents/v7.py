"""Kaggriculture agent v7 - goose engine, payback-gated expansion, melon waves (days 0/5/10).
...
P = {
    ...
    "goose_life_end": 26,    # a goose bought on day d lays from day d+3 through day 28
    "fert_weight": 0.5,      # fertilizer price decays over a goose's life; value it at half
    "land_fill": 12,         # geese a new quadrant must pay back across
    "melon_days": (0, 5, 10),    # days a melon wave is planted
    "melon_n": 12,           # tiles per melon wave
    "melon_spare": 4,        # empty tiles to keep for geese/wheat beside a wave
    "melon_max_q": 3,        # never buy land for melons beyond this many quadrants
}


def fib(k):
    """k-th hire cost with farmHandCostMult=1: 1, 1, 2, 3, 5, 8, ..."""
    a, b = 1, 1
    for _ in range(k - 1):
        a, b = b, a + b
    return a
...
def agent(obs, config=None):
    ...
    # ---------------- survey ----------------
    m_seed = seeds.get("MELON", 0)
    if MEM.get("mday") != day:
        MEM["mday"], MEM["mplan"] = day, []
    empties = [p for p in tile_order(n) if tiles[p[1]][p[0]] is None]
    mplan = [p for p in MEM["mplan"] if tiles[p[1]][p[0]] is None][:m_seed]
    if len(mplan) < m_seed:
        # melon tiles come from the far end of the newest land, away from the goose core
        mplan += [p for p in reversed(empties) if p not in mplan][:m_seed - len(mplan)]
    MEM["mplan"] = mplan
    geese, ecoops, eg, ew, wheat, weeds, melons = [], [], [], [], [], [], []
    for (x, y) in tile_order(n):
        t = tiles[y][x]
        if t == "LOCKED" or (x, y) in mplan:
            continue
        r = role(x, y, n)
        if t is None:
            ...
            weeds.append((x, y))
        elif t["kind"] == "PLANT" and t["crop"] == "MELON":
            melons.append((x, y))
        elif t["kind"] == "PLANT":
            wheat.append((x, y))
        elif "animal" in t:
    ...
    def melon_ops(p):
        """Water on planting day and every other day until age 6, then daily; harvest at 6 units."""
        t = tiles[p[1]][p[0]]
        age = day - t["planted_day"]
        L = []
        if not t["watered_today"] and (age >= 6 or t["consecutive_unwatered"] >= 1) and age <= 12:
            L.append("WATER")
        if age >= 10 and (t["yield_units"] >= 6 or age >= 12 or day >= 28 or
                          (t["watered_today"] and t["yield_units"] >= 5)):
            L.append("HARVEST")
        return L
    ...
    for p in melons:
        work[p] = ("W", melon_ops(p))
    for p in mplan:
        work[p] = ("W", ["PLANT_MELON", "WATER"])
    ...
    if MEM["day"] != day:
        MEM["day"] = day
        MEM["owner"] = {}
    # hires ordered at hours 0-1 arrive by hour 2; after that, split work over units that exist
    planned = len(units) if hour >= 2 else max(MEM.get("planned", 1), len(units))
    ...
    for ui, pos in enumerate(units):
        ...
        if pos in SH:
            ...
            spare_w = cw - need_w
            if inv.get("MELON"):
                act = ["PLACE", "MELON", inv["MELON"]]
            elif eggs and (eggs >= 3 or not mine or steps_left < 24):
                act = ["PLACE", "EGG", eggs]
        ...
        if inv.get("MELON", 0) >= 6 and not cands:
            target, ops = home(pos), None
        ...
        if ops is None:
            if inv.get("MELON"):
                acts.append(["PLACE", "MELON", inv["MELON"]])
            elif inv.get("EGG"):
    ...
    cash = money
    if hour == 0 and day in P["melon_days"] and P["melon_n"] > 0:
        # melon wave: buy land first if the farm lacks empty tiles, then the seeds
        k = P["melon_n"]
        nq = len(me["unlocked_quadrants"])
        if len(empties) - len(mplan) < k + P["melon_spare"] and nq < P["melon_max_q"] \
                and cash >= LAND_PRICES[nq - 1] + 80 * k:
            orders.append(["BUY_LAND"])
            cash -= LAND_PRICES[nq - 1]
        k = min(k, int(cash // 80))
        if k > 0:
            orders.append(["BUY_SEED", "MELON", k])
            cash -= 80 * k
    ...
    if hour <= 1 and day <= 29:
        if hour == 0:
            wl = len(geese) * P["w_goose"] + (len(wheat) + len(ew) + len(melons) + len(mplan)) * P["w_wheat"] \
    ...
    if hour <= 12 and day <= P["last_goose_day"]:
        ...
        nq = len(me["unlocked_quadrants"])
        wl += g_wait * P["w_goose"]
        life = max(0, P["goose_life_end"] - day)

        def goose_profit(extra):
            """Season profit of the (extra+1)-th new goose at today's prices, net of its price."""
            units = (wl + (extra + 1) * P["w_goose"]) / P["turns_per_unit"]
            turn_cost = fib(max(1, int(units + 0.999) - 1)) / P["turns_per_unit"]
            daily = 2 * prices["EGG"] + P["fert_weight"] * fert_price - wp - P["w_goose"] * turn_cost
            return daily * life - GOOSE_COST

        if free < 3 and nq < 4 and day <= P["last_land_day"] and wl + 16 * P["w_goose"] < cap:
            price = LAND_PRICES[nq - 1]
            if cash - reserve >= price + 2 * GOOSE_COST and \
                    goose_profit(P["land_fill"] - 1) * P["land_fill"] > price:
                orders.append(["BUY_LAND"])
                cash -= price
                free += 16
        k = int((cash - reserve) // (GOOSE_COST + wp))
        k = min(k, free, int(max(0, cap - wl) / P["w_goose"]))
        while k > 0 and goose_profit(k - 1) <= 0:
            k -= 1
        if hour > 0:
            k = min(k, 4)
        if k > 0:
